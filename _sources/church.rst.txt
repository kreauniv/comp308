...
3. If L is a λ-term, then ``(λ (<id>) L)`` is also a λ-term. 

.. admonition:: **Terminology: Free variables**

    In a λ-term, any identifier (a.k.a. "variable" in this context, due to its
...
**η-reduction**
    ...
    That is "eta reduction"

    .. code:: racket

        (λ (x) (F x)) = F

    A λ-term ``F`` is considered equivalent to ``(λ (x) (F x))`` provided the
    identifier ``x`` does not occur as a free variable in ``F``. Note that all
...
:rkt:`(λ (y) (y y))`, but to (subject to α-renaming) :rkt:`(λ (z) (y z))`. This
is because the substitution must be valid for all equivalences and that means
it must be valid if we'd first α-renamed the inner λ as :rkt:`(λ (z) (x z))`
before performing the β-reduction.

All the equivalences need to be taken into account when interpreting a
given λ-term. All the relations described in "structure" run both ways
between the concerned λ-terms.
