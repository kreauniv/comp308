...
(define-type SimpleVal (U Integer Boolean Symbol))
(struct Var ([name : Symbol]) #:transparent)
(struct Val ([v : SimpleVal]) #:transparent)

(define-type Unifiable (U Var Val))

(: samevar? (-> Var Var Boolean))
(define (samevar? A B) (eqv? A B))

(struct Binding ([key : Var] [val : Unifiable]) #:transparent)
...
(: extend (-> Var Unifiable BSet BSet))
(define (extend A val bset)
  (cons (list A val) bset))


(: lookup (-> Var BSet (U False Binding)))
(define (lookup v bs)
  (if (empty? bs)
      #f
      (if (eqv? (Binding-key (first bs)) v)
          (first bs)
          (lookup v (rest bs)))))
...
(: walk (-> Unifiable BSet Unifiable))
(define (walk A bset)
  (let ([m (and (Var? A) (lookup A bset))])
    (if m
        (walk (second m) bset)
        A)))
...
(: unify/basic (-> Unifiable Unifiable BSet (U False BSet)))
...
(: unify-list-v1 (-> Unifiable Unifiable BSet (U False BSet)))
